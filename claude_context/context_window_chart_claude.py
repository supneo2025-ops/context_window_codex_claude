#!/usr/bin/env python3
"""
Generate context window usage chart for Claude Code sessions.
Shows token usage over time with user message annotations.

Adapted from codex_context for Claude Code sessions.
"""
        ...
        for message_index, line in enumerate(f, 1):
            try:
                ...
                if record_type == 'user':
                    ...
                    # Skip short messages if min_message_length is set
                ...
                # Extract token counts from assistant messages
                    ...
                    if usage:
                        ...
                        token_data.append({
                            ...
                            'model_context_window': 200000,  # Claude default
...
def format_duration(seconds: float) -> str:
    """Format duration in (Xs), (Xm Ys), or (Xh Ym)."""
    seconds = int(seconds)
...
def list_recent_sessions(hours: float = 24) -> None:
    """List Claude sessions modified within the last N hours."""
    session_files = find_session_files()

    if not session_files:
        print(f"No session files found in {CLAUDE_ROOT}")
        return
    ...
    for session_file in session_files:
        ...
        if mtime >= cutoff_ts:
            recent_sessions.append((session_file, mtime))
    ...
    print(f"Claude Code sessions within the last {hours:g} hour(s):")
    for index, (session_path, mtime) in enumerate(recent_sessions, 1):
        ...
        print(
            f"  {index}. {project} | {session_path.name} | "
...
def generate_html(token_data: List, user_messages: List, output_path: Path,
    ...
    # Match user messages to token counts
    ...
    # Prepare time-based and message-based data
    ...
    model_context_window = token_data[0]['model_context_window'] if token_data else 200000
    ...
    # Calculate cost and duration for each user message
    ...
    # Generate chat message cards HTML
    chat_cards_html = ""
...
<head>
  ...
  <title>Claude Code Context Window - {session_id}</title>
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Generate context window usage charts for Claude Code sessions',
...
def main():
    ...
    if args.list is not None:
        list_recent_sessions(args.list)
        return
    ...
    if args.latest:
        ...
        if session_path.exists():
            ...
            # Try to find by UUID
            session_files = find_session_files()
            matching = [f for f in session_files if args.session_file.lower() in f.name.lower()]
            if not matching:
                print(f"No session found matching: {args.session_file}")
                return
            matching.sort(key=lambda p: p.stat().st_mtime, reverse=True)
            session_file = matching[0]
            print(f"Found session: {session_file}")
    else:
        print("Please specify --latest or a session file/UUID")
        return

    # Extract and generate
    print(f"\nAnalyzing session: {session_file.name}")
    if args.min_length > 0:
        print(f"  Filtering messages shorter than {args.min_length} characters")
    token_data, user_messages, total_msg_count = extract_session_data(session_file, args.min_length)
    print(f"  Found {len(token_data)} token events")
